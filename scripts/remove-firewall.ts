...
async function main() {
    ...
    const firewallAddress = hre.ethers.ZeroAddress;
    
    for (const [contract, address] of Object.entries(networkConfig.networks[hre.network.name].contracts)) {
        logger.info(` -> Removing from ${colors.cyan(contract)} at address ${colors.cyan(address as string)}`);
